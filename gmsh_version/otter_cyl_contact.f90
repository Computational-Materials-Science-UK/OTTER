...
contains
    ...
    subroutine new_contact(max_rad,spheres,cyls,curr_cyl,cylinder_num,num_contact,contact_pt,curr_pt,&
        i_pt,i_num,box)
        implicit none
        ...
        real(kind=8),intent(in) :: max_rad,box ! max_radius

        !local parameters
        integer :: i,j,k,dir_j,jj
        real(kind=8),dimension(4) :: distance
        logical :: debug=.false.,contact,check_contact,in
            ...
            if (check_contact) then !Do a detailed check only if all ranges overlap
                ...
                if ((b(1,1) .le. 1.) .and. (b(1,1) .ge. 0.) .and. (b(2,1) .le. 1.) .and. (b(2,1) .ge. 0.)) then
                  ...
                  do n=0,8
                    ...
                    do jj=1,3
                        if ((cyls(curr_cyl+n,jj).gt.box+cyls(curr_cyl+n,4)).and.((cyls(curr_cyl+n,jj+5) &
                        +cyls(curr_cyl+n,4)).gt.box)) in =.false.
                        if ((cyls(curr_cyl+n,jj).lt.0.0-cyls(curr_cyl+n,4)).and.(cyls(curr_cyl+n,jj+5) &
                        .lt.0.0-cyls(curr_cyl+n,4))) in =.false.
                        if ((cyls(i+n,jj).gt.box+cyls(i,4)).and.((cyls(i+n,jj+5) &
                        +cyls(i,4)).gt.box)) in =.false.
                        if ((cyls(i+n,jj).lt.0.0-cyls(i,4)).and.(cyls(i+n,jj+5) &
                        .lt.0.0-cyls(i+n,4))) in =.false.
                    end do
                    if (in) then
                        open(unit=18,file='contacts.txt',Access = 'append',status='old',action='write')
                        if (contact) then
                            write(18,*)curr_cyl,' ',i
                        end if 
                        close(18)
