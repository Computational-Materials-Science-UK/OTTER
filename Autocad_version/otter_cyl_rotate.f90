...
contains
    ...
    subroutine otter_cyl_rotate_wrapper(r,cyls,i,nn_count,q_pt,rotation_count)
        ...
        real(kind=8),dimension(3)       :: p_uvec,qq,cyl_diff
        ...
        if ((abs(r(1)-center_of_gravity(1)).lt.0.9).and.(abs(r(2)-center_of_gravity(2)).lt.0.9)) then 
            cyl_diff=cyls(i,6:8)-cyls(i,1:3)     
            cyls(i,1)=cyls(i,1)+0.1*cyl_diff(1)
            cyls(i,2)=cyls(i,2)+0.1*cyl_diff(2)
            cyls(i,3)=cyls(i,3)+0.1*cyl_diff(3)
        ...
        if(debug)write(*,*) '[Rotate wrapper] Post COG check length check: ',cyls(i,9),norm2(cyls(i,6:8)-cyls(i,1:3))
        !Set up v
...
subroutine rotate_beck(r,qq,p,q,rp,p_uvec,pmag,qmag,center_of_gravity,debug)
    ...
    logical                               :: align
    real(kind=8)                :: d,mag_unit_PQ,mag_unit_fiber,theta,length,length_original,lenght_tol
    real(kind=8),dimension(4,4) :: T,T_inv,Rx, Rx_inv, Ry, Ry_inv,Rz,Rz_neg
    real(kind=8),dimension(3) :: unit_fiber,unit_PQ,CoG,unit_cyl
...
!Defining X,Y,Z for end point 1
    ...
    length_original=norm2((endpoint_one(1:3)-endpoint_two(1:3)))
    lenght_tol=0.0001
...
if((length.lt.9.9999))then
    unit_cyl=(endpoint_one(1:3)-endpoint_two(1:3))/length
    endpoint_two(1:3)=endpoint_one(1:3)+(length_original*unit_cyl(1:3))
endif
length=norm2((endpoint_one(1:3)-endpoint_two(1:3)))
